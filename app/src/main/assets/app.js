...
(function () {
  ...
  var THEMES = [
    { id: "elder-dark",    name: "Elder Ease Dark",  a: "#0b1016", b: "#7c9f45" },
    { id: "elder-light",   name: "Elder Ease Light", a: "#f7f2e7", b: "#6f843b" },
  ...
  var DEFAULTS = {
    ...
    showCopy: true,
    showShare: false,                  // share hidden by default
  ...
  var I18N = {
    en: {
      ...
      theme_elder_dark:"Elder Ease Dark", theme_elder_light:"Elder Ease Light", theme_dark_ambient:"Dark Ambient", theme_gold_navy:"Gold & Navy", theme_haram_light:"Haram Light", theme_green_classic:"Green Classic", theme_high_contrast:"High Contrast",
    ...
    ur: {
      ...
      theme_elder_dark:"بزرگ آسانی — ڈارک", theme_elder_light:"بزرگ آسانی — لائٹ", theme_dark_ambient:"ڈارک ایمبیئنٹ", theme_gold_navy:"سنہرا اور نیوی", theme_haram_light:"حرم لائٹ", theme_green_classic:"سبز کلاسک", theme_high_contrast:"ہائی کنٹراسٹ",
  ...
  var CATEGORY_UR = {
    ...
    "Istighfar":"استغفار", "Salawat":"درود و سلام"
  };
  var TYPE_UR = { "Dua":"دعا", "Azkar":"ذکر", "Kalima":"کلمہ", "Istighfar":"استغفار", "Salawat":"درود" };
  var TITLE_UR = {};
  function localizeCategory(v) { return state.settings.lang === "ur" ? (CATEGORY_UR[v] || TYPE_UR[v] || v || "") : (v || ""); }
  function localizeType(v) { return state.settings.lang === "ur" ? (TYPE_UR[v] || CATEGORY_UR[v] || v || "") : (v || ""); }
  ...
  function load() {
    try {
      ...
      if (!("showUrdu" in raw)) s.showUrdu = true;
  ...
  function suppressCardBismillah(it) {
    if (!it) return true;
    if (it.suppress_card_bismillah === true) return true;
    return normalizeArabic(it.arabic).indexOf(BASMALAH_NORM) === 0;
  }
  function escHtml(v) {
  ...
  // Non-Qur'anic reading flow: display-only phrase segmentation using the text's
  // own punctuation. No Qur'anic Waqf symbols are invented and the stored Arabic
  // is never altered — long supplications simply read one clear phrase per line.
  function renderArabicReading(text) {
    text = String(text || "");
    var segs = text.split(/[\u060C\u061B\u06D4\.]+/).map(function (s) { return s.trim(); }).filter(Boolean);
    if (segs.length < 2) return escHtml(text);
    return segs.map(function (s) { return '<span class="ar-phrase">' + escHtml(s) + '</span>'; }).join("");
  }
  ...
  function render() {
    ...
    arEl.style.fontFamily = AR_FONTS[s.arabicFont] || AR_FONTS.scheherazade;
  ...
  function fitContent(arEl, arStart, tlStart, trStart) {
    ...
    if (autoFit) {
      ...
      var over = function () { return sc.scrollHeight > sc.clientHeight + 1; };
  ...
  function shareCardPng(it, cb) {
    try {
      ...
      if (!suppressCardBismillah(it)) addBlock("بِسْمِ اللّٰهِ الرَّحْمٰنِ الرَّحِيمِ", "500 38px serif", 56, accent, "center", true, 16, 1);
  ...
  function onKey(e) {
    // While a sheet is open, let native D-pad move between its controls; Back/Escape closes.
    if (anySheetOpen()) {
      if (e.key === "Escape" || e.key === "GoBack" || e.key === "BrowserBack") { closeSheets(); e.preventDefault(); }
      return;
    }
    if (!state.nav) state.nav = { zone: READER_ZONE, col: 0 };
  ...
  function buildSettings() {
    ...
    disp.appendChild(segRow(t("language"), t("languageDesc"), "lang",
      [[t("optEnglish"), "en"], [t("optUrdu"), "ur"]]));
    var tg = document.createElement("div"); tg.className = "theme-grid";
    THEMES.forEach(function (theme) {
      var c = document.createElement("button");
      c.className = "theme-card"; c.setAttribute("data-theme-id", theme.id);
      var tk = "theme_" + theme.id.replace(/-/g, "_");
      c.innerHTML = '<div class="swatch"><div class="a" style="background:' + theme.a + '"></div><div class="b" style="background:' + theme.b + '"></div></div><div class="tname">' + esc(t(tk)) + '</div>';
      c.addEventListener("click", function () { state.draft.theme = theme.id; applyTheme(theme.id); markThemes(); });
      tg.appendChild(c);
    });
    disp.appendChild(rowCustom(t("theme"), t("themeDesc"), tg, true));
    disp.appendChild(toggleRow(t("simpleMode"), t("simpleModeDesc"), "easyView"));
    disp.appendChild(selectRow(t("arabicTypeface"), t("arabicTypefaceDesc"), "arabicFont",
      [[t("fontScheherazade"), "scheherazade"], [t("fontAmiri"), "amiri"], [t("fontReem"), "reemkufi"], [t("fontNastaliq"), "nastaliq"]]));
    disp.appendChild(segRow(t("arabicWeight"), t("arabicWeightDesc"), "arabicWeight",
      [[t("optRegular"), "regular"], [t("optThick"), "thick"]]));
    disp.appendChild(stepperRow(t("arabicTextSize"), t("arabicTextSizeDesc"), "arScale"));
    disp.appendChild(stepperRow(t("translationFont"), t("translationFontDesc"), "trScale"));
    disp.appendChild(stepperRow(t("transliterationFont"), t("transliterationFontDesc"), "tlScale"));
    body.appendChild(disp);

    // CONTENT
    var cont = group(t("grpContent"));
    cont.appendChild(toggleRow(t("showArabic"), t("showArabicDesc"), "showArabic"));
    cont.appendChild(toggleRow(t("showTranslit"), t("showTranslitDesc"), "showTranslit"));
    cont.appendChild(toggleRow(t("showEnglish"), t("showEnglishDesc"), "showEnglish"));
    cont.appendChild(toggleRow(t("showUrdu"), t("showUrduDesc"), "showUrdu"));
    cont.appendChild(toggleRow(t("showSource"), t("showSourceDesc"), "showSource"));
    cont.appendChild(toggleRow(t("showPauseMarks"), t("showPauseMarksDesc"), "showPauseMarks"));
    cont.appendChild(toggleRow(t("showWaqfLegend"), t("showWaqfLegendDesc"), "showWaqfLegend"));
    if (state.draft && state.draft.showWaqfLegend) {
      var lg = document.createElement("div"); lg.className = "legend-wrap"; lg.innerHTML = waqfLegendHtml();
      cont.appendChild(rowCustom(t("waqfLegend"), t("waqfLegendDesc"), lg, true));
    }
    cont.appendChild(toggleRow(t("tajweed"), t("tajweedDesc"), "tajweed"));
    body.appendChild(cont);

    // NAVIGATION
    var nav = group(t("grpNavigation"));
    nav.appendChild(segRow(t("defaultFlow"), t("defaultFlowDesc"), "flowMode",
      [[t("optMixed"), "mixed"], [t("optByCategory"), "category"]]));
    nav.appendChild(segRow(t("autoRotation"), t("autoRotationDesc"), "autoRotate",
      [[t("optOn"), true], [t("optOff"), false]]));
    nav.appendChild(segRow(t("rotationInterval"), t("rotationIntervalDesc"), "interval",
      [["15s", 15], ["25s", 25], ["40s", 40], ["60s", 60]]));
    nav.appendChild(toggleRow(t("copyButton"), t("copyButtonDesc"), "showCopy"));
    nav.appendChild(toggleRow(t("shareButton"), t("shareButtonDesc"), "showShare"));
    ...
    locNote.textContent = t("liveLocationNote");
    pr.appendChild(rowCustom(t("location"), t("locationDesc"), locNote, false));
    body.appendChild(pr);

    // ABOUT — opens a separate about.html page (not shown inside the dashboard)
    var ab = group(t("grpAbout"));
    var aboutBtn = document.createElement("button");
    aboutBtn.className = "btn-about";
    aboutBtn.type = "button";
    aboutBtn.textContent = t("openAbout");
    aboutBtn.addEventListener("click", function () {
      var w = null;
      try { w = window.open("about.html", "_blank"); } catch (e) { w = null; }
      if (!w) { try { window.location.href = "about.html"; } catch (e2) {} }
    });
    ab.appendChild(aboutBtn);
    body.appendChild(ab);
  }
  ...
  function toggleRow(name, desc, key) {
    ...
    inp.addEventListener("change", function () {
      ...
      if (key === "showArabic" || key === "showTranslit" || key === "showEnglish" || key === "showUrdu" || key === "showSource" || key === "showPauseMarks") previewFonts();
    });
  ...
  function segRow(name, desc, key, opts) {
    ...
    opts.forEach(function (o) {
      ...
      b.addEventListener("click", function () {
        ...
        if (key === "arabicScript" || key === "arabicWeight" || key === "lang") previewFonts();
      });
      seg.appendChild(b);
    });
